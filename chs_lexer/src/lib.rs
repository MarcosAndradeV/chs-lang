...
impl TokenKind {
    fn from_word_or_keyword(value: &str) -> Self {
        match value {
            ...
            | "type" | "cast" | "end" | "syscall" => Self::Keyword,
            _ => Self::Ident,
        }
    }
...
impl Lexer {
    ...
    fn character(&mut self) -> Token {
        ...
        self.read_char();
        match self.ch {
            b'\'' => return self.make_token(TokenKind::Invalid, "Empty character literal"),
            b'\\' => {
        ...
        if self.ch != b'\'' {
            return self.make_token(TokenKind::Invalid, "Unclosed character literal");
        }
        self.read_char();
        Token {
            value: buf,
