...
use std::{env::Args, fs::File, io::Write, os::unix::process::CommandExt, path::PathBuf, process::{self, exit, ExitCode}};
        ...
        Some(("compile", flags, args)) => {
            if let Some(file_path) = args.get("INPUT").cloned() {
                match flags.get("-o").cloned() {
                    Some(out_path) => {
                        compile(file_path, Some(out_path), flags.is_present("-r"));
                    }
                    None  => {
...
fn compile(file_path: String, outpath: Option<String>, run: bool) -> CHSResult<()> {
    ...
    let result = fasm_proc.wait_with_output().expect("Failed to wait for fasm");
    ...
    if run {
        ...
        let result = run_proc.wait_with_output().expect("Failed to wait for fasm");
        if !result.status.success() {
            chs_error!("Failed {}", String::from_utf8_lossy(&result.stderr));
        }
