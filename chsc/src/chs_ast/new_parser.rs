...
use super::ast::{
    Ast, AstNode, Expression, FunctionDecl, LetStatement, Param, Params, Precedence,
...
impl<'a> Parser<'a> {
    ...
    fn parse_type(&mut self) -> ParseResult<CHSType> {
        let p = self.peek();
        match p.kind {
            TokenKind::Identifier if &p.source == "int" => {
                Ok(CHSType::Int)
            }
            _ => {
                return Err(ParseError::Expected(p.loc, format!("Type but got {}", p)));
            }
        }
    }
