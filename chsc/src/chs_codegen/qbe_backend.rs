...
impl<'src> QBEBackend<'src> {
    ...
    fn generate_function(&mut self, f: MIRFunction) {
        let name = &self.raw_module[&f.name];
        let arguments = f
            .params
            .into_iter()
            .map(|local_id| {
                let Local { name, ty } = &f.locals[local_id.0];
            ...
            .collect();
        let return_ty = {
            if f.return_type == CHSType::Void {
                None
            } else {
                Some(self.convert_type(f.return_type))
            }
        };
        ...
        for block in f.blocks {
            ...
            for stmt in block.statements {
                match stmt {
                    mir::Statement::Assign { target, value } => {
                        let Local { name, ty } = &f.locals[target.0];
                        if let Some(name) = name {
                            let temp = Value::Temporary(self.raw_module[name].to_string());
                            let ty = self.convert_type(ty.clone());
                            b.assign_instr(temp, ty, self.instr_from_rvalue(value));
                        } else {
                            b.add_instr(self.instr_from_rvalue(value));
                        }
                    }
                    mir::Statement::Store { .. } => todo!(),
                }
            }
            match block.terminator {
                ...
                Terminator::Return(Some(operand)) => {
                    let (_, value) = self.convert_operand(operand);
    ...
    fn convert_operand(&mut self, operand: Operand) -> (Type<'src>, Value) {
        match operand {
    ...
    fn instr_from_rvalue(&mut self, value: Rvalue) -> Instr<'src> {
        match value {
            Rvalue::Use(..) => todo!(),
